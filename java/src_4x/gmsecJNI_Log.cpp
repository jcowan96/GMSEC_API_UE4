...
{
	Log::setReportingLevel((LogLevelGMSEC) level);
}
  ...
  (JNIEnv *jenv, jclass jcls, jint jLevel, jlong jLogHandlerPtr)
{
	...
	Log::registerHandler((LogLevelGMSEC) jLevel, logHandler);
}
...
{
	jstring s = makeJavaString(jenv, Log::toString((LogLevelGMSEC) jLevel));
