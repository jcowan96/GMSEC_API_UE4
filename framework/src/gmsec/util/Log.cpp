...
LogLevelGMSEC Log::fMaxLevel = logNONE;
...
void Log::RegisterHandler(LogLevelGMSEC level, GMSEC_LOGGER_HANDLER* handler)
{
...
public:
	...
	std::ostream& get(LogLevelGMSEC level)
	{
		setup(level);
		return *oss;
	}
	...
	void setup(LogLevelGMSEC level)
	{
...
std::ostream& Logger::Get(LogLevelGMSEC level)
{
	API_4x_LoggerAdapter* logger = dynamic_cast<API_4x_LoggerAdapter*>(handler);
...
LogStream& LoggerStream::Get(LogLevelGMSEC level)
{
