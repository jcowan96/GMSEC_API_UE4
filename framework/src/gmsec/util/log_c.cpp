...
void CALL_TYPE gmsec_SetReportingLevel(enum LogLevelGMSEC level)
{
	Log::SetReportingLevel(level);
}

enum LogLevelGMSEC CALL_TYPE gmsec_GetReportingLevel()
{
	return Log::GetReportingLevel();
}

GMSEC_BOOL CALL_TYPE gmsec_IsLoggingEnabled(enum LogLevelGMSEC level)
{
...
void CALL_TYPE gmsec_RegisterLogHandlerByLevel(enum LogLevelGMSEC level, GMSEC_LOGGER_HANDLER *handler)
{
	Log::RegisterHandler(level, handler);
}

const char* CALL_TYPE gmsec_LogLevelGMSECToString(enum LogLevelGMSEC level)
{
	return Log::ToString(level);
}

enum LogLevelGMSEC CALL_TYPE gmsec_LogLevelGMSECFromString(const char *level)
{
	return Log::FromString(level);
}

void CALL_TYPE gmsec_CAPILog(enum LogLevelGMSEC level, char *fmt, ...)
{
	char buffer[2046];
