...
//
LogLevelGMSEC InternalLog::s_maxLevel = logNONE;


void InternalLog::setReportingLevel(LogLevelGMSEC level)
{
...
void InternalLog::registerHandler(LogHandler* handler)
{
	...
	{
		registerHandler((LogLevelGMSEC) level, handler);
	}
}


void InternalLog::registerHandler(LogLevelGMSEC level, LogHandler* handler)
{
...
void InternalLog::registerHandler(GMSEC_LogLevelGMSEC level, GMSEC_LogHandler* handler)
{
	GMSEC_DEBUG << "Registering handler " << (const void*) handler << " for level " << Log::toString(level);
...
LogLevelGMSEC InternalLog::fromString(const char* level)
{
...
const char* InternalLog::toString(LogLevelGMSEC level)
{
