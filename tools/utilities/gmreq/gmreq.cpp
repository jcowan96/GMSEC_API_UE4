...
private:
	...
	void request(Message& msg, int msg_timeout_ms);
	void request(const std::string& subject, int msg_timeout_ms);
};
...
bool GMSEC_Request::run()
{
	...
	try
	{
		...
		if (msg.get())
		{
			...
			request(*(msg.get()), msg_timeout_ms);
		}
		else
		{
			// Send canned request message
			request(subject, msg_timeout_ms);
		}
	}
	catch (Exception& e)
	{
...
void GMSEC_Request::request(Message& request, int msg_timeout_ms)
{
	...
	if (request.getField("RESPONSE") == NULL || request.getIntegerValue("RESPONSE") == 1)
	{
		...
		Message* reply = connection->request(request, msg_timeout_ms, -1);
...
void GMSEC_Request::request(const std::string& subject, int msg_timeout_ms)
{
	//o Create message
