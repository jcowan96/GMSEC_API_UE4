...
{
	gmsec::api::util::Log::setReportingLevel(static_cast<LogLevelGMSEC>(level));
}
...
void Log::RegisterHandler(LoggingLevel level, LogHandler^ handler)
{
	if (handler == nullptr)
	{
		gmsec::api::util::Log::registerHandler(static_cast<LogLevelGMSEC>(level), reinterpret_cast<gmsec::api::util::LogHandler*>(NULL));
	}
	else
	{
		THROW_EXCEPTION_IF_NULLPTR(handler->GetUnmanagedImplementation(), StatusClass::OTHER_ERROR, StatusCode::INVALID_CALLBACK, "LogHandler implementation is null");

		gmsec::api::util::Log::registerHandler(static_cast<LogLevelGMSEC>(level), handler->GetUnmanagedImplementation());
	}
}
...
{
	return gcnew String(gmsec::api::util::Log::toString(static_cast<LogLevelGMSEC>(level)));
}
